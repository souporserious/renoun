...
describe('healMarkdown', () => {
  test.concurrent('returns original input when there are no special markers', () => {
    const source = 'Just some plain text with no markdown markers.'
    expect(healMarkdown(source)).toBe(source)
  })

  test.concurrent('returns empty string when input is empty', () => {
    expect(healMarkdown('')).toBe('')
  })

  test.concurrent('completes an incomplete link at the end of the input', () => {
  ...
  test.concurrent('drops an incomplete image at the end of the input while preserving prefix text', () => {
  ...
  test.concurrent('closes unbalanced inline code at the tail', () => {
    expect(healMarkdown('`code')).toBe('`code`')
    expect(healMarkdown('`already closed`')).toBe('`already closed`')
  })

  test.concurrent('closes unbalanced block math at the tail', () => {
    const source = '$$ a + b'
    const result = healMarkdown(source)
  ...
  test.concurrent('closes unbalanced inline math delimiters', () => {
  ...
  test.concurrent('closes multiple nested inline math delimiters in LIFO order', () => {
  ...
  test.concurrent('closes unmatched emphasis markers on the last line only', () => {
  ...
  test.concurrent('does not treat list item markers as emphasis', () => {
  ...
  test.concurrent('does not treat word_internal_underscores as emphasis', () => {
  ...
  test.concurrent('closes unmatched underscore emphasis when not inside a word', () => {
  ...
  test.concurrent('closes unmatched strikethrough markers', () => {
  ...
  test.concurrent('heals a trailing unclosed single dollar math expression', () => {
  ...
  test.concurrent('heals a single dollar math expression with whitespace', () => {
    const input = 'Here is $ a + b'
    const expected = 'Here is $ a + b$'
    expect(healMarkdown(input)).toBe(expected)
  })

  test.concurrent('does not modify an already closed single dollar expression', () => {
    const input = 'The value is $x=5$. Next sentence.'
    expect(healMarkdown(input)).toBe(input)
  })

  test.concurrent('ignores escaped dollars (\\$)', () => {
  ...
  test.concurrent('ignores dollars inside inline code', () => {
    // The scanner should prioritize code blocks over math
    const input = 'Use the variable `$foo` in bash'
    expect(healMarkdown(input)).toBe(input)
  })

  test.concurrent('ignores dollars inside code blocks', () => {
    const input = '```\nconst price = $500;\n```'
    expect(healMarkdown(input)).toBe(input)
  })

  test.concurrent('correctly distinguishes between inline ($) and block ($$)', () => {
  ...
  test.concurrent('handles multiple inline math segments correctly', () => {
    const input = 'First $a$, then $b$, finally $c'
    const expected = 'First $a$, then $b$, finally $c$'
    expect(healMarkdown(input)).toBe(expected)
  })

  test.concurrent('works alongside LaTeX style parens', () => {
