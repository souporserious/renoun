...
- d9c0939: Adds `getExportValue` method to `Directory` that will attempt to load the requested export from either the `index` or `readme` paths. This makes it simpler to use with entries that also include `JavaScriptFile` and `MDXFile`.
