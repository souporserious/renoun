    ...
    case 'gitHost': {
      ...
      if (!gitSource) {
        if (process.env.NODE_ENV !== 'production') {
          console.error(
            '[renoun] Logo missing git.source. Context git:',
            config.git
          )
        }
        throwGitSourceError('Logo')
      }

      const gitHost = config.git?.host ?? getGitHostFromUrl(gitSource!)
      const sharedProps = {
        width: '24',
        height: '24',
        role: 'img',
        fill,
        style: css as any,
      } as React.SVGProps<SVGSVGElement> & { css?: CSSObject }
      ...
      if (gitHost === 'gitlab') {
        return (
          ...
          >
            ...
            />
            <path
              d="M9.9082 27.1407L13.1834 29.625L15.1774 31.1354C15.4117 31.3124 15.6972 31.4082 15.9908 31.4082C16.2844 31.4082 16.5697 31.3124 16.8042 31.1354L18.7982 29.625L22.0801 27.1407C22.0801 27.1407 19.2452 25.0005 15.9883 22.5378L9.9082 27.1407Z"
              fill="#FCA326"
            />
            <path
      ...
      throw new Error(`[renoun] Unsupported git host: ${gitHost}`)
    }
    default:
      throw new Error(
        `[renoun] Unsupported Logo variant: ${(props as { variant: string }).variant}`
      )
  }
}
