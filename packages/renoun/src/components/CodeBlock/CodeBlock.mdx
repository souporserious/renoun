...
```tsx
import { CodeBlock as RenounCodeBlock, Tokens } from 'renoun/components'
import styles from './CodeBlock.module.css'

export function CodeBlock(props: React.ComponentProps<typeof RenounCodeBlock>) {
  return (
    <CodeBlock
...
<Note>
  Not every component can be styled using the `css`, `className`, and `style`
  props. See the following section for fully customizing the rendering of the
  `CodeBlock` component.
</Note>
