...
When `source` is a file with multiple exports, `Reference` renders them in order in separate sections. Pass a specific `JavaScriptModuleExport` directly if you only want to render a single export.
