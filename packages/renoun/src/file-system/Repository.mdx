...
const repo = Repository.local() // defaults to "."
...
const remoteRepo = Repository.remote({
...
const scopedRepo = Repository.remote({
...
- For remote repositories, cloning writes data to disk. Use `cacheDirectory` if you want to isolate cache storage.
- Use `clone: false` to avoid cloning and read via the host API.
