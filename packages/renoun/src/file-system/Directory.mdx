...
const repo = Repository.remote('owner/repo')
...
Use `getTree()` when you need navigational structure, such as sidebars or static params:

```ts
const navigation = await docs.getTree()
```

Use `getStructure()` when you need a lightweight flat index for search, manifests, feeds, or sitemap-like exports:

```ts
const searchIndex = await docs.getStructure({
...
For payload-sensitive indexes, `includeDescriptions` accepts `false`, `'snippet'`, or `'full'`, `includeGitDates` accepts `false`, `'first'`, `'last'`, or `'both'` (`true` remains an alias for `'both'`), file `authors` are opt-in via `includeAuthors: true`, and header-mode exports omit `tags` unless you pass `includeTags: true`.

`getStructure()` already returns a flat array of directory and file structures, so `getTree()` remains the dedicated navigation API and no extra `flatten` option is needed today.
