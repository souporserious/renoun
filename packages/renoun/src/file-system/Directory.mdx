...
For payload-sensitive indexes, `includeGitDates` accepts `false`, `'first'`, `'last'`, or `'both'` (`true` remains an alias for `'both'`). File `authors` are opt-in via `includeAuthors: true`, and header-mode exports omit `tags` unless you pass `includeTags: true`.
