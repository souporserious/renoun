...
- name: Restore renoun cache
  ...
  with:
    ...
    key: ${{ runner.os }}-node${{ matrix.node-version }}-renoun-cache-${{ github.workflow }}-${{ steps.renoun-cache-token.outputs.token }}-${{ hashFiles('pnpm-lock.yaml') }}
    restore-keys: |
      ${{ runner.os }}-node${{ matrix.node-version }}-renoun-cache-${{ github.workflow }}-${{ steps.renoun-cache-token.outputs.token }}-
```
