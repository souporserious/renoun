...
const clonedRepo = Repository.remote({
...
// Remote API (virtual)
const virtualRepo = Repository.remote({
  path: 'https://github.com/mrdoob/three.js',
  clone: false,
})
...
// Local
const localRepo = Repository.local()
...
By default, remote repositories are cloned into `<workspaceRoot>/.renoun/cache/git`.
...
#### Cache persistence availability

renoun works correctly even when SQLite is unavailable. SQLite is an optional upgrade used for persistent cache reuse across process restarts and better cache coordination across workers.

If SQLite (`node:sqlite`) is missing or fails to initialize, renoun automatically falls back to in-memory cache behavior. Queries and metadata remain accurate, but cache state becomes process-local and non-persistent.

For metric names, tags, and recommended dashboard/alert starters, see [SQLite Cache Telemetry Reference](./SQLiteCacheTelemetry.mdx).

#### Deterministic CI cache keys

Use `renoun cache-token` to print a stable token for renoun's persistent file-system cache:

```bash
v1-schema2-analysis0-structure0-git0-gitvirtual0
```

The token changes only when renoun cache versions change.

When renoun is already installed in a project:

```bash
pnpm exec renoun cache-token
```

In GitHub Actions, resolve the token once and use it in `actions/cache`:

```yaml
- name: Resolve renoun cache token
  id: renoun-cache-token
  run: echo "token=$(renoun cache-token)" >> "$GITHUB_OUTPUT"

- name: Restore renoun cache
  ...
  with:
    path: |
      ...
      .next/cache/renoun/fs-cache.sqlite-shm
      .next/cache/renoun/fs-cache.sqlite-wal
    key: ${{ runner.os }}-node${{ matrix.node-version }}-renoun-cache-${{ steps.renoun-cache-token.outputs.token }}-${{ hashFiles('pnpm-lock.yaml') }}-${{ github.sha }}
    restore-keys: |
      ${{ runner.os }}-node${{ matrix.node-version }}-renoun-cache-${{ steps.renoun-cache-token.outputs.token }}-${{ hashFiles('pnpm-lock.yaml') }}-

- name: Truncate renoun cache WAL before the cache post-job save
