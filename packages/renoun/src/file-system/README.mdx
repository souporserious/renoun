...
- name: Resolve renoun cache token
  ...
  run: echo "token=$(node packages/renoun/src/cli/index.ts cache-token)" >> "$GITHUB_OUTPUT"
...
- name: Restore renoun cache
  ...
  with:
    ...
    key: ${{ runner.os }}-node${{ matrix.node-version }}-renoun-cache-${{ steps.renoun-cache-token.outputs.token }}-${{ hashFiles('pnpm-lock.yaml') }}
    restore-keys: |
      ${{ runner.os }}-node${{ matrix.node-version }}-renoun-cache-${{ steps.renoun-cache-token.outputs.token }}-${{ hashFiles('pnpm-lock.yaml') }}-
      ${{ runner.os }}-node${{ matrix.node-version }}-renoun-cache-${{ steps.renoun-cache-token.outputs.token }}-
```
