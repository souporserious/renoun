...
const renounMdx = new Package({ name: '@renoun/mdx' })

// Resolve the "remark/add-headings" export from the manifest
const remarkAddHeadings = await renounMdx.getExport('remark/add-headings')

// Drill into a specific binding that export exposes
const defaultExport = remarkAddHeadings?.getExport('default')
const addHeadings = await defaultExport?.getRuntimeValue()

addHeadings?.getType() //=> exposes the schema/type information for that export
```
