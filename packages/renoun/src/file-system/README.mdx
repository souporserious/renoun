...
const fileSystem = new GitHostFileSystem({
...
### Using `LocalGitFileSystem`

The `LocalGitFileSystem` adapter provides high-performance file access for local
...
```ts
import { Directory, LocalGitFileSystem } from 'renoun'

const fileSystem = new LocalGitFileSystem({
...
```ts
import { LocalGitFileSystem } from 'renoun'
...
#### Comparison with `GitHostFileSystem`

| Feature       | LocalGitFileSystem    | GitHostFileSystem |
| ------------- | --------------------- | ----------------- |
