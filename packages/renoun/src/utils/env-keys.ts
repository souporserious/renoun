export const PROCESS_ENV_KEYS = {
  nodeEnv: 'NODE_ENV',
  ci: 'CI',
  vitest: 'VITEST',
  vitestWorkerId: 'VITEST_WORKER_ID',
  renounDebug: 'RENOUN_DEBUG',
  renounDebugAbsPath: 'RENOUN_DEBUG_ABS_PATH',
  renounDebugPackagePath: 'RENOUN_DEBUG_PACKAGE_PATH',
  renounDebugDirectorySnapshotRestore: 'RENOUN_DEBUG_DIRECTORY_SNAPSHOT_RESTORE',
  renounDebugRestoredScope: 'RENOUN_DEBUG_RESTORED_SCOPE',
  renounServerId: 'RENOUN_SERVER_ID',
  renounServerPort: 'RENOUN_SERVER_PORT',
  renounServerHost: 'RENOUN_SERVER_HOST',
  renounServerClientRpcCache: 'RENOUN_SERVER_CLIENT_RPC_CACHE',
  renounServerClientRpcCacheTtlMs: 'RENOUN_SERVER_CLIENT_RPC_CACHE_TTL_MS',
  renounServerClientRefreshNotifications:
    'RENOUN_SERVER_CLIENT_REFRESH_NOTIFICATIONS',
  renounServerRefreshNotifications: 'RENOUN_SERVER_REFRESH_NOTIFICATIONS',
  renounServerRefreshNotificationsEffective:
    'RENOUN_SERVER_REFRESH_NOTIFICATIONS_EFFECTIVE',
  renounAnalysisWatchers: 'RENOUN_ANALYSIS_WATCHERS',
  renounAnalysisClientRpcCache: 'RENOUN_ANALYSIS_CLIENT_RPC_CACHE',
  renounAnalysisClientRpcCacheTtlMs: 'RENOUN_ANALYSIS_CLIENT_RPC_CACHE_TTL_MS',
  renounAnalysisRefreshNotifications: 'RENOUN_ANALYSIS_REFRESH_NOTIFICATIONS',
  renounPrewarmWorkerPayload: 'RENOUN_PREWARM_WORKER_PAYLOAD',
  githubSponsorsToken: 'GITHUB_SPONSORS_TOKEN',
  renounBuildProfile: 'RENOUN_BUILD_PROFILE',
  renounBuildProfileFile: 'RENOUN_BUILD_PROFILE_FILE',
} as const
