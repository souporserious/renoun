...
export interface ExportSource<Value> extends BaseSource {
  ...
  /**
   * The runtime value of the export loaded from a dynamic import map generated in the `.omnidoc` directory at the root of the project.
...
export interface CollectionOptions<Exports extends FileExports> {
   ...
   * the hostname (e.g. `/docs` in `https://omnidoc.com/docs`).
   */
  basePath?: string
      ...
      if (parseExportValue) {
        try {
          ...
          if (error instanceof Error) {
            throw new Error(
              `[omnidoc] Failed to parse export "${name}" using schema for file "${this.source.getPath()}" \n\n${
                error.message
              }`,
              { cause: error }
            )
  ...
  implements FileSystemSource<AllExports>
{
  ...
  getPath() {
    ...
    if (!calculatedPath) {
      throw new Error(
        `[omnidoc] Could not calculate depth. Source path not found for file path "${
          this.#sourcePath
        }".`
      )
    }
  ...
  getOrder() {
    ...
    if (order === undefined) {
      throw new Error(
        `[omnidoc] Source file order not found for file path "${
          this.#sourcePath
  ...
  async getSources({ depth = Infinity }: { depth?: number } = {}) {
    if (!isValidDepth(depth)) {
      throw new Error(
        `[omnidoc] Invalid depth "${depth}" provided for source at path "${this.getPath()}". Depth must be a positive integer or Infinity.`
      )
    }
  ...
  > {
    if (!isValidDepth(depth)) {
      throw new Error(
        `[omnidoc] Invalid depth "${depth}" provided for source siblings at path "${this.getPath()}". Depth must be a positive integer or Infinity.`
      )
    }
...
You can fix this error by taking one of the following actions:
  ...
  getSourceFile() {
    ...
    throw new Error(
      `[omnidoc] Expected a source file but got a directory at "${
        this.#sourcePath
      }".`
    )
  }
  ...
  async getModuleExports() {
    ...
    if (!getImport) {
      throw new Error(
        `[omnidoc] No source found for path "${this.getPath()}" at file pattern "${
  ...
  implements CollectionSource<AllExports>
{
    ...
    if (this.options.sort) {
      try {
        ...
        if (error instanceof Error && error.message.includes(badge)) {
          throw new Error(
            `[omnidoc] Error occurred while sorting sources for collection with file pattern "${
              this.filePattern
  ...
  async getSources({ depth = Infinity }: { depth?: number } = {}) {
    if (!isValidDepth(depth)) {
      throw new Error(
        `[omnidoc] Invalid depth "${depth}" provided for collection with file pattern "${this.filePattern}". Depth must be a positive integer or Infinity.`
      )
    }
 ...
 * Note, an import getter for each file extension will be generated at the root of the project in a `.omnidoc/collections.ts` file.
 *
