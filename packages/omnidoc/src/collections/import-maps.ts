...
export const PACKAGE_NAME = 'omnidoc/core'
export const PACKAGE_DIRECTORY = '.omnidoc'
...
/** Updates the tsconfig.json file to include the `omnidoc` package alias in the `paths` field. */
function codemodTsConfigPaths(project: Project) {
  ...
  if (typeof tsconfigFilePath === 'string') {
    ...
    const omnidocPath = tsconfigJson.compilerOptions.paths['omnidoc/*']

    if (omnidocPath === undefined || omnidocPath[0].endsWith('js')) {
      tsconfigJson.compilerOptions.paths['omnidoc/*'] = [
...
export async function generateCollectionImportMap(filename?: string) {
  ...
  /* Update the tsconfig.json file to include the `omnidoc` package alias in the `paths` field. */
