...
```tsx
import { CodeBlock as OmnidocCodeBlock, Tokens } from 'omnidoc/components'
import styles from './CodeBlock.module.css'

export function CodeBlock(
  props: React.ComponentProps<typeof OmnidocCodeBlock>
) {
  return (
    <CodeBlock
...
If you need more customization, the `CodeBlock` component can be fully overridden by importing it from `omnidoc/components` and extending it:
