...
export async function getTokens(
  ...
  if (highlighter === null) {
    throw new Error(
      '[omnidoc] Highlighter was not initialized. Ensure that the highlighter is created before calling "getTokens".'
    )
  }
...
function getQuickInfo(
  ...
  const displayText = displayParts
    ...
    // Finally, replace the in-memory omnidoc directory
    .replaceAll('/omnidoc', '')
...
function throwDiagnosticErrors(
  ...
  const workingDirectory = join(process.cwd(), 'omnidoc', posix.sep)
  ...
  throw new Error(
    `[omnidoc] ${componentName} type errors found ${
