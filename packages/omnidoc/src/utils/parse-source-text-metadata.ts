...
export async function parseSourceTextMetadata({
  ...
  if ('value' in props) {
    ...
    if (isRelative && !workingDirectory) {
      throw new Error(
        `[omnidoc] The "workingDirectory" prop is required for "${componentName}" with the relative source "${props.source}".\n\nPass a valid [workingDirectory]. If this is being renderend directly in an MDX file, make sure the "omnidoc/remark" plugin is configured correctly.`
      )
    }
  ...
  // TODO: reimplement formatting that allows for other formatters (this should move to .omnidoc directory config e.g. `getFormatter`)
  ...
  if ('value' in props) {
    filename = join('omnidoc', filename)
  }
  ...
  if (isJavaScriptLikeLanguage && !filename.includes('.')) {
    if (!finalLanguage) {
      throw new Error(
        `[omnidoc] The "language" prop was not provided to the "${componentName}" component and could not be inferred from the filename. Pass a valid "filename" with extension or a "language" prop`
      )
    }
  ...
  if (isJavaScriptLikeLanguage) {
    try {
      ...
      if (error instanceof Error) {
        ...
        throw new Error(
          `[omnidoc] Error trying to create "${componentName}" source file at working directory "${workingDirectory}"`,
          { cause: error }
        )
      }
    }
  ...
  const filenameLabel = (filenameProp || filename)
    .replace(join('omnidoc', posix.sep), '') // Remove omnidoc/ prefix
