...
export function getExportedDeclaration(
  ...
  if (exportDeclarations.length > 1) {
    ...
    if (filteredExportDeclarations.length > 1) {
      ...
      throw new Error(
        `[omnidoc] Multiple declarations found for export after filtering type aliases, interfaces, and property access expressions in source file at ${filePath}. Only one export declaration is currently allowed. Please file an issue for support.`
      )
    }
