...
export class WebSocketClient {
  ...
  #handleError(event: WebSocket.ErrorEvent) {
    throw new Error(
      `[omnidoc] WebSocket client error: ${event.message} \n\nThis was most likely caused by the "omnidoc" server not running.`
    )
  }
  ...
  #retryConnection() {
    if (this.#currentRetries < this.#maxRetries) {
      ...
      setTimeout(() => {
        console.log(
          `[omnidoc] Attempting to reconnect to WebSocket server... (${this.#currentRetries}/${this.#maxRetries})`
        )
        this.#connect()
      }, this.#retryInterval)
    } else {
      throw new Error(
        `[omnidoc] Could not reconnect to the WebSocket server after ${this.#maxRetries} attempts.`
      )
    }
  }
  ...
  callMethod(method: string, params: any, timeout = 60000): Promise<any> {
    ...
    return new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        reject(
          new Error(
            `[omnidoc] Timed out after one minute for the following request: ${JSON.stringify(request)}`
          )
        )
        delete this.#requests[id]
      }, timeout)
