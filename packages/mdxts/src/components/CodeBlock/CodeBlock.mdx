...
If you need full customization, the `CodeBlock` component can be fully overridden by importing it from `mdxts/components` and extending it:
...
<Note>
  This will only render the tokens, use the other `CodeBlock` components like
  `LineNumbers`, `LineHighlights`, and `Toolbar` to render the other parts of
  the code block.
</Note>
