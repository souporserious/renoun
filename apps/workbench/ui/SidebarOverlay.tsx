...
export function SidebarOverlay({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 z-40 bg-black/20 backdrop-blur-sm transition-[opacity,backdrop-filter] md:hidden ${open ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}
