...
Then we need to install the `renoun` package dependencies:
...
export default async function Page(props: PageProps<'/components/[...slug]'>) {
  ...
  const fileExports = await component.getExports()

  return fileExports.map((fileExport) => {
    return (
      <div key={fileExport.name}>
        <h2>{fileExport.name}</h2>
        <p>{fileExport.description}</p>
      </div>
    )
  })
}
...
Adjust the regular expression to match the files you want to ignore. For example, to _exclude_ test files for a TSX component:
...
Alternatively, you can choose what to _include_ like example files:

```tsx
function getComponentExample(path: string) {
  return import(
