...
Now, we'll create a schema using `zod` for the frontmatter of an MDX file:
...
We can now apply the Zod `frontmatterSchema` to your `Directory` using the `schema` option:
