...
A component named `CodeBlock` is required to be passed to the MDX renderer. The component can simply render the [`Code` block variant](/components/code-block) from `renoun`.
...
Or import and map the `Code` component to a `CodeBlock` MDX component:
...
export default function Page() {
  return (
    <GettingStarted components={{ CodeBlock: Code }} />
  )
}
```
