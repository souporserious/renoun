...
<Command variant="install">valibot</Command>
...
### Validating MDX sections

When using the MDX remark plugin that [auto-exports `sections`](/guides/mdx#remark-add-sections), TypeScript does not know the shape of that export unless you include it in your schema.

In this scenario, we manually type the exported `sections` in our schema so validation and type inference work as expected:

<CodeBlock path="guides/valibot.ts" />
