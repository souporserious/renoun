...
To generate metadata for each page, we can export a `metadata` constant that returns an object with the metadata properties:
...
To use the metadata, we can get the value of the `metadata` constant and use it to set the page title and description:
...
export default async function Page({ params }) {
  ...
  const metadata = await Component.getNamedExport('metadata').getValue()
...
To generate metadata for collections, we can use root directory in the targeted file pattern to set the metadata:

```tsx filename="components/index.ts"
export const metadata = {
  title: 'Components',
...
```tsx filename="components/page.tsx"
