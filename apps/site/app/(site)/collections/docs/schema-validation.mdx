...
We can now apply the Zod `frontmatterSchema` to your collection using the `schema` option in the `createCollection` utility:
...
We can now apply the Valibot `frontmatterSchema` to your collection using the `schema` option in the `createCollection` utility:
