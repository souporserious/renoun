...
### Caching Figma images

When using the [`Image` component](/docs/components/image) with Figma sources, downloads are cached to `public/images` by default. To change where those files are written (for example to commit them under a different directory), set the `images.outputDirectory` option on `RootProvider` to either a project-relative or absolute path:

```tsx
<RootProvider images={{ outputDirectory: 'public/design-assets' }} />
```

The cache path you choose is used for both reading and writing files and should point to a location that your framework serves as static assets.
...
Most VS Code themes publish their source on GitHub in repositories that follow the structure of the extension. You can browse the extension's Marketplace listing, click **Repository**, and then look for a `themes` directory that contains one or more JSON files. Download the theme JSON you want to use, add it to your project, and point `RootProvider` at the file path.

Once the file is in your project, run [`renoun theme`](/docs/cli#pruning-theme-files) to strip out metadata that renoun does not need at runtime. This keeps the bundle small while preserving all of the colors required by the theme loader.

### Pruning custom themes

Many third-party themes include metadata that renoun does not need at runtime. You can trim those files down with the renoun CLI so they stay as small as possible while keeping the required shape for the theme loader. Run the `theme` command and pass it the path to your JSON file:

<Command variant="run">theme path/to/theme.json</Command>

The command will validate the file, remove unused keys, and write the minimized JSON back to disk in place. Because the file is overwritten, it's a good idea to keep the theme under version control or create a backup before pruning it.
...
Use the `languages` prop to define which grammars are available to `CodeBlock`.
