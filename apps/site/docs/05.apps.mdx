...
2. **Override application**
   Your files are applied on top using an additive model—files you create are linked into the
...
3. **Framework launch**
   ...
   directory** using the framework binary resolved from the runtime's dependency graph. The
   framework sees the merged result: base app + your customizations.

4. **Path resolution**
   renoun sets an internal environment variable so its utilities resolve paths relative to the
   runtime directory, ensuring your overridden content is discovered correctly.

### Dependency resolution

In app mode, the runtime directory symlinks the template app's `node_modules`, so the template
defines the primary dependency graph. Your overrides can still import packages that you installed
only in your project because Node will walk up parent directories (eventually reaching your
project's `node_modules`) if a package isn't found in the runtime.

To make the runtime's "view" match this mental model, renoun merges your project's dependency
sections into the runtime `package.json` (without installing anything new).
...
2. **Applies your overrides** using an **additive model** that intelligently merges your files with the
...
2. **Apply your overrides on top**
   - Any files or directories you were overriding (like `posts/` or `ui/RootProvider.tsx`) are
...
Creating an app template is straightforward—it's just a normal application with a few conventions.
Here's how to build one that works well with renoun's override system.
...
User files are merged additively with your template, so your defaults show up alongside user
content. Design your template to work well with this model.
...
See the [`@renoun/blog`](https://github.com/souporserious/renoun/tree/main/apps/blog) source for
...
- **Links point to your real files.** When you override a file, the framework reads directly from
