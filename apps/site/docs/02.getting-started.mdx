...
In this getting started guide, Next.js will be used as an example. However, renoun works with any framework or bundler that supports React Server Components. For additional examples, please see the [guides section](/guides).

## Installation

First, install renoun using your preferred package manager and additionally `next` or a compatible framework if you haven't already:
...
With this configuration, you can now import files using the `@/` alias which we will use in the upcoming examples.

## MDX Content

The renoun toolkit helps with authoring MDX using the `renoun/mdx` package, allowing you to write content with a mix of Markdown and React components. It is not required to use, but provides a set of [useful plugins](/guides/mdx#pre-configured-plugins).
...
The `Directory` class is a core utility in renoun. This allows you to easily query and render files and directories within a file system. To create a list of blog posts, query all of the MDX files in the `posts` directory we created earlier:
...
Now we can use the configured directory to render the contents of our MDX files by querying them from the file system. Create a new file in the `app/posts` directory called `[slug].tsx` and add the following:
...
Use the `loaders` option in the `Directory` constructor to assign a specific loader for each file type. In the `posts` directory, for instance, you can use a dynamic import as the loader to ensure that the MDX files are processed by the bundler:
...
Below is an example demonstrating how to validate a `frontmatter` object exported from a module using [Zod](https://zod.dev/):
...
Here we define a schema for the front matter of our MDX files and use it to validate each file in the `posts` directory. This ensures that each file adheres to the schema and provides type safety when accessing the `frontmatter` export:
...
The renoun toolkit provides several built-in components to enhance your documentation like [`APIReference`](/components/api-reference) and [`CodeBlock`](/components/code-block). For example, you can use the `APIReference` component to document all exports from a module:
