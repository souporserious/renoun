...
The `createSource` utility collects metadata about source files which can be used to generate routes. The task of handling routing is delegated to the respective frameworks. The following examples demonstrate how to set up routing using `createSource` in Next.js.
...
Use the `paths` method to generate static routes for your documentation:

```tsx allowErrors
export function generateStaticParams() {
  return posts.paths().map((pathname) => ({ slug: pathname }))
}
```
