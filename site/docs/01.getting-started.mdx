...
Use the `createSource` function exported from the `mdxts` package to generate metadata and helpers for MDX and TypeScript source files that can be used to build routes for content and documentation:
...
Use the `get` method from the modules loader to retrieve the metadata for a specific source file and render it:
