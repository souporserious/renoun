...
The `theme` option is used for syntax highlighting code blocks and is powered by [Shiki](https://shiki.matsu.io/). The `theme` value should correspond to either a [built-in](https://shiki.matsu.io/themes) theme or a source path to a custom JSON theme that adheres to the [VS Code theme specification](https://code.visualstudio.com/api/extension-guides/color-theme).
...
Allowing errors is useful to show incomplete or incorrect code examples in documentation for educational purposes. It's highly encouraged to disallow errors if possible so code examples are always using correct code that can compile.
